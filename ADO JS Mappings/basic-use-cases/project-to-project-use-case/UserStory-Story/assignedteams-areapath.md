...
const getSourceEntity = async (tpEntity) => {
  return await tpApiV2
    .getByIdAsync(tpEntity.entityType, Number(tpEntity.sourceId), {
      select: `{
        art:assignedteams.Select(team.agilereleasetrain.name).first()
...
if (team) {
  ...
  if (!tpTeam) {
    console.warn(`Failed to find Team "${team}" in Targetprocess`);
...
const tpApiV2 = context.getService("targetprocess/api/v2");
const [rootNode = null, teamNode = null] = args.targetFieldValue
